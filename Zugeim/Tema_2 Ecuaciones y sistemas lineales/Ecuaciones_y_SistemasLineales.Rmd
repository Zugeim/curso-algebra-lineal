...
% DOS ECUACIONES CON 2 INCOGNITAS
    ...
    x = linspace(-5,10,100); % Voy a crear 100 puntos espaciados en 10
    y1 = (1-2*x)/2;
    y2 = 2+x;
    plot(x,y1,x,y2) % Le ira dando valores a las x de la y
    % Donde intersectan representa la unica solución del sistema

% TRES ECUACIONES CON 2 INCOGNITAS
    A = [4 2; 1 -2; 3 4];
    b = [3;2;1];
    AB = [A b];

    % 1º Despejamos las x2 (se podria tambien despejas las x1)
    %%%% x2 = (3-4*x1)/2
    %%%% x2 = (x1-2)/2
    %%%% x2 = (1-3*x1)/4
    ...
    x2b = (x1-2)/2;
    x2c = (1-3*x1)/4;
    plot(x1,x2a,x1,x2b,x1,x2c); % Le ira dando valores a las x de la y
    % Donde intersectan representa la unica solución del sistema
...
% TRES ECUACIONES CON 3 INCOGNITAS
    ...
    % Da a 'x' e 'y' valores desde 0 a 3 en pasos de 0.1
    [x,y] = meshgrid(0:0.1:3);
    
    % 2º Para cada pareja de 'x' e 'y' declaro 'z'
    
        hold on % Inicio de un dibujo

        z = (9-x-y)/2;
        mesh(x,y,z); % Creo una malla con la 'x' e 'y' y la 'z' que acabo de contruir
        ...
        hold off % Fin de un dibujo

        view(120,30) % Con esto se representa todo lo anterior con un tamaño de 120 x 30
```
...
rref(AB) % Nos da la funcion rref aplicada a la matriz ampliada
         %es decir, nos da el resultado aplicando Gauss
```
...
rango = rank(A)
rank(A) == rank(AB) % ans = 1 quiere decir True
...
% Comprobamos los rangos
rank(A) == rank(AB) % ans = 0 quiere decir False
...
% Si usasemos rref obtendriamos esto
rref(AB) % 0 no puede ser igual a 1, asi que es incompatible
```

## Ecuaciones matriciales.
```{matlab}
% AX + 3B = (C+D)X + 3D + 10I 
% Hay que llegar a la forma M*X = N
% Solucionado -> (A-(C+D))X = 3(D-B) + 10I
